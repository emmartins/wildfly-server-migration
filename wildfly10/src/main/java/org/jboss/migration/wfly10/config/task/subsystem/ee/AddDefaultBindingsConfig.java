 ...
 * @author emmartins
 */
public class AddDefaultBindingsConfig<S> extends UpdateSubsystemConfigurationSubtaskBuilder<S> {
    ...
    protected ServerMigrationTaskResult updateConfiguration(ModelNode config, S source, SubsystemConfiguration subsystemConfiguration, TaskContext context, TaskEnvironment taskEnvironment) {
        ...
        final boolean addConcurrencyUtilitiesDefaultConfigSkipped = new TaskEnvironment(migrationEnvironment, org.jboss.migration.wfly10.config.task.subsystem.EnvironmentProperties.getSubsystemSubtaskPropertiesPrefix(subsystemConfiguration.getResourceName(), AddConcurrencyUtilitiesDefaultConfig.TASK_NAME)).isSkippedByEnvironment();
        ...
        setupDefaultDatasource(defaultDataSourceJndiName, defaultDataSourceName, addOp, subsystemConfiguration.getParentResource(), context, taskEnvironment, taskResultBuilder);
        setupDefaultJMSConnectionFactory(defaultJmsConnectionFactoryJndiName, defaultJmsConnectionFactoryName, addOp, subsystemConfiguration.getParentResource(), context, taskEnvironment, taskResultBuilder);
    ...
    private void setupDefaultJMSConnectionFactory(String defaultJmsConnectionFactoryJndiName, String defaultJmsConnectionFactoryName, final ModelNode addOp, SubsystemConfiguration.Parent subsystemResources, final TaskContext context, final TaskEnvironment taskEnvironment, final ServerMigrationTaskResult.Builder taskResultBuilder) {
        // if the subsystem config defines expected default resource then use it
        final SubsystemConfiguration resource = subsystemResources.getSubsystemConfiguration(SubsystemNames.MESSAGING_ACTIVEMQ);
        if (resource == null) {
            return;
        }
    ...
    private void setupDefaultDatasource(String defaultDataSourceJndiName, final String defaultDataSourceName, final ModelNode addOp, SubsystemConfiguration.Parent subsystemResources, final TaskContext context, final TaskEnvironment taskEnvironment, final ServerMigrationTaskResult.Builder taskResultBuilder) {
        // if the subsystem config defines expected default resource then use it
        final SubsystemConfiguration resource = subsystemResources.getSubsystemConfiguration(SubsystemNames.DATASOURCES);
        if (resource == null) {
            return;
        }
