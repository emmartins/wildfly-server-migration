...
/**
 * A task which migrates JMS Bridges present in a Messaging subsystem config.
 * @author emmartins
 */
public class JMSBridgesModulesFinder implements MigrateModules.ModulesFinder {
    @Override
    ...
    @Override
    public void processElement(XMLStreamReader reader, MigrateModules.ModuleMigrator moduleMigrator) throws IOException {
        final String namespaceURI = reader.getNamespaceURI();
        if (namespaceURI == null || !namespaceURI.startsWith("urn:jboss:domain:"+ SubsystemNames.MESSAGING_ACTIVEMQ)) {
            return;
        }
        moduleMigrator.migrateModule(reader.getAttributeValue(null, "module"), "jms-bridge references it");
    }
