 ...
 * @author emmartins
 */
public class SubsystemConfigurationCompositeTask<S> extends ManageableResourceCompositeTask<S, SubsystemConfiguration> {

    protected SubsystemConfigurationCompositeTask(ManageableResourceCompositeTask.BaseBuilder<S, SubsystemConfiguration, ?> builder, S source, Collection<? extends ManageableResource> resources) {
        super(builder, source, resources);
    }

    public interface SubtaskExecutor<S> extends ManageableResourceCompositeTask.SubtaskExecutor<S, SubsystemConfiguration> {
    }
    ...
    protected static abstract class BaseBuilder<S, B extends BaseBuilder<S, B>> extends ManageableResourceCompositeTask.BaseBuilder<S, SubsystemConfiguration, B> {
        protected BaseBuilder(ServerMigrationTaskName taskName) {
            super(taskName, SubsystemConfiguration.class);
        }
    }
    ...
    public static class Builder<S> extends BaseBuilder<S, Builder<S>> {
        ...
        public ServerMigrationTask build(S source, Collection<? extends ManageableResource> resources) {
            return new SubsystemConfigurationCompositeTask<>(this, source, resources);
        }
    }
}