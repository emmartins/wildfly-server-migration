 ...
 * @author emmartins
 */
public interface SingleResource<S, R extends ManageableResource> extends ServerConfigurationBuildParameters<S> {
    R getResource();

    static <S, T extends ManageableResource, R extends ManageableResource> BuildParameters.Mapper<? extends ResourcesCollection<S, T>, SingleResource<S, R>> from(ManageableResourceSelector<R> rSelector) {
        return stManageableResourcesCollection -> {
            ...
            final Collection<R> rCollection = rSelector.fromResources(stManageableResourcesCollection.getResources());
            return rCollection.stream()
                    .map(rResource -> new SingleResource<S, R>() {
                        @Override
