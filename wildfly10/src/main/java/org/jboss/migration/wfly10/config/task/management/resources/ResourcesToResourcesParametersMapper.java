...
public class ResourcesToResourcesParametersMapper<S, T extends ManageableResource, R extends ManageableResource> implements BuildParameters.Mapper<ResourcesBuildParameters<S, T>, ResourcesBuildParameters<S, R>> {
    ...
    @Override
    public Collection<ResourcesBuildParameters<S, R>> apply(ResourcesBuildParameters<S, T> tParams) {
        return Collections.singleton(new ResourcesBuildParametersImpl<S, R>(tParams.getSource(), tParams.getServerConfiguration(), resourceSelector.fromResources(tParams.getResources())));
    }
}
