...
public class ServerConfigurationToResourcesParametersMapper<S, T extends ManageableResource, R extends ManageableResource> implements BuildParameters.Mapper<ServerConfigurationBuildParameters<S>, ResourcesBuildParameters<S, R>> {
    ...
    @Override
    public Collection<ResourcesBuildParameters<S, R>> apply(ServerConfigurationBuildParameters<S> params) {
        return Collections.singleton(new ResourcesBuildParametersImpl<S, R>(params.getSource(), params.getServerConfiguration(), resourceSelector.fromResources(params.getServerConfiguration())));
    }
}
