 ...
 * @author emmartins
 */
public class WildFly10FullServer extends AbstractServer {
    ...
    public void migrate(Server source, ServerMigrationContext context) throws IOException {
        final WildFly10FullMigration migration = getMigration(source);
        if (migration != null) {
            migration.migrate(source, this, context);
        } else {
            throw new IllegalArgumentException("Source server migration to WildFly 10 Full not supported: "+source.getProductInfo());
        }
    }

    protected WildFly10FullMigration getMigration(Server source) {
