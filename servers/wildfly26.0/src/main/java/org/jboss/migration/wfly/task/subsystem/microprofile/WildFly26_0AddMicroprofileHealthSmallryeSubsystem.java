 ...
 * @author emmartins
 */
public class WildFly26_0AddMicroprofileHealthSmallryeSubsystem<S> extends AddSubsystemResources<S> {

    public WildFly26_0AddMicroprofileHealthSmallryeSubsystem() {
    ...
    static class SubtaskBuilder<S> extends AddSubsystemResourceSubtaskBuilder<S> {
        SubtaskBuilder() {
            super(JBossSubsystemNames.MICROPROFILE_HEALTH_SMALLRYE);
        ...
        protected void addConfiguration(ManageableResourceBuildParameters<S, SubsystemResource.Parent> params, TaskContext taskContext) {
            ...
            op.get("empty-liveness-checks-status").set(new ValueExpression("${env.MP_HEALTH_EMPTY_LIVENESS_CHECKS_STATUS:UP}"));
            op.get("empty-readiness-checks-status").set(new ValueExpression("${env.MP_HEALTH_EMPTY_READINESS_CHECKS_STATUS:UP}"));
