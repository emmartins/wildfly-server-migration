...
/**
 * The class PosixParser provides an implementation of the {@link Parser#flatten(Options,String[],boolean) flatten} method.
 *
 * @version $Id$
...
public class PosixParser extends Parser {
    ...
    /**
     * Resets the members to their original state i.e. remove all of <code>tokens</code> entries and set <code>eatTheRest</code>
     * to false.
     */
    private void init() {
        eatTheRest = false;
     ...
     * </p>
     * <ol>
     * <li>if <code>stopAtNonOption</code> is <b>true</b> then do not burst anymore of <code>arguments</code> entries, just add
     ...
     * <li>if the current <code>arguments</code> entry is "<b>-</b>" just add the entry to the list of processed tokens</li>
     * <li>if the current <code>arguments</code> entry is two characters in length and the first character is "<b>-</b>" then
     * check if this is a valid {@link Option} id. If it is a valid id, then add the entry to the list of processed tokens and
     * set the current {@link Option} member. If it is not a valid id and <code>stopAtNonOption</code> is true, then the
     * remaining entries are copied to the list of processed tokens. Otherwise, the current entry is ignored.</li>
     * <li>if the current <code>arguments</code> entry is more than two characters in length and the first character is
     * "<b>-</b>" then we need to burst the entry to determine its constituents. For more information on the bursting algorithm
     * see {@link PosixParser#burstToken(String, boolean) burstToken}.</li>
     * <li>if the current <code>arguments</code> entry is not handled by any of the previous rules, then the entry is added to
     * the list of processed tokens.</li>
     * </ol>
     *
     ...
     * <li>if an {@link Option} does <b>NOT</b> exist <b>AND</b> <code>stopAtNonOption</code> <b>IS</b> set then add the special
     * token "<b>--</b>" followed by the remaining characters and also the remaining tokens directly to the processed tokens
     * list.</li>
     * <li>if an {@link Option} does <b>NOT</b> exist <b>AND</b> <code>stopAtNonOption</code> <b>IS NOT</b> set then add that
